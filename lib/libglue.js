      ...
      if( !Glue._inited ) {
		...
		load: function( x, script, vars ) {
				...
				while( (e = script.indexOf( "\n" )) > -1 ) {
				  ...
				  if( line.startsWith( ":" ) ) {
				    labels[line.substr( 1 )] = cmp.length;
				    continue;
				  }
				  row = { "conditions": [] };
				  // any if terms?
				  ...
				  if( label ) {
					...
					if( conds && (conds.length > 0) ) {
					  for( ci = 0; ci < conds.length; ci++ ) {
					    if( !GlueEval._toBool( ((ts in vars) ? vars[ts] : '0') ) ) {
							  runline = 0;
							  break;
					    }
						}
			...
			_exec: function( gluex, cmd, args, ondonegoto, onerrorgoto ) {
					if( window.glueApps ) {
						if( cmd in window.glueApps ) {
							return window.glueApps[cmd]['glueExec'].call( this, gluex, args, ondonegoto, onerrorgoto, args );
						}
					}
					return 0;
				},
